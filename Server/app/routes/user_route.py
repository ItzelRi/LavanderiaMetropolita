...
def create_user():
    ...
    #COMMIT REGISTRA UN CAMBIO EN LA DB COMO GITHUB
    db.session.commit()
...
def login():
    ...
    return jsonify({"msg":"Algo salio mal al intenter iniciar sesion, revise sus claves"}), 400

@user_bp.route("/logout/<int:user_id>", methods=["POST"])
def logout(user_id):
    logout_user(user_id)
    return jsonify({"msg":"Log out exitoso"}), 200
