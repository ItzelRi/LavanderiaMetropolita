...
def update_client(client_id, updated_data):
    client = Client.query.get(client_id) #get es solo para id
    if not client:
        return None
    
    for key, value in updated_data.items():
        setattr(client, key, value) #espera clave (name, phone, email, etc) y valor de client para actualizar. Primero que quierea actualizar y despues que
    db.session.commit()
    return client
