...
export const ListO =()=>{ 
      const [AllOrders, setdataSearched] = useState([])
    ...
    try {
      console.log("Hola, soy la paginacion", Pagination)
        const AllOrders = await axios.get(`https://7qnhlz7j-5000.usw3.devtunnels.ms/orders/get-orders-dashboard?pagination=${Pagination}`)
        setdataSearched(AllOrders.data)
    } catch (error) {
        console.log(error)
        }
      }
    ...
    const mapped = AllOrders.map((register)=>( 
    [ register.user_name,register.client_name,register.created_at,register.state,register.total,
      (<>
...
return (
        ...
        <View style={styles.container}>
            <View style={styles.nav}>
              <Text style={styles.title}>Ordenes Generales</Text>
            </View>

            <ScrollView style={styles.mainContent}>
              <Table style={styles.tabla}> 
                <Row data={header}/>
                <Row data={[ 'Vendedor', 'Cliente', 'Fecha de orden', "Estado", "Total","No.Orden",]} style={styles.head} textStyle={styles.text}/>
